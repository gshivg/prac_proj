<div class="flex flex-row items-center justify-center h-full">
    <div class="w-[33%] h-min p-4 gap-2 bg-white-700 rounded-md bg-clip-padding backdrop-filter backdrop-blur-md bg-opacity-30 border border-gray-100">
        <h1 class="text-xl">Add Stock</h1>
        <form wire:submit="save" class="flex flex-col gap-2 mt-2">
            <flux:dropdown class="w-full">
                <flux:button class="w-full" icon-trailing="chevron-down">Select Product to Add</flux:button>

                <flux:menu>
                    <flux:menu.radio.group wire:model="product">
                    @foreach ($products as $product)
                        <flux:menu.radio>{{$product}}</flux:menu.radio>
                    @endforeach
                    </flux:menu.radio.group>
                </flux:menu>
            </flux:dropdown>
            <flux:input wire:model="product" label="Selected Product" placeholder="Select Product" disabled type="text" icon="lock-closed" />

            <flux:input wire:model="amount" label="Stock Amount" type="number" icon="inbox-stack" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1').replace(/^0[^.]/, '0');"/>
            ...
            <div x-data="{
                ...
                datePickerFormatDate(date) {
                    ...
                    datePickerCalculateDays();
                    @this.manufacturing_date = datePickerValue;
                " x-cloak>
                <div class="container">
                    <div class="w-full">
                        <label for="datepicker" class="block mb-1 text-sm font-medium text-neutral-300">Select Manufacturing Date</label>
                        <div class="relative w-full">
                            ...
                            <div
                                ...
                                @click="@this.manufacturing_date = datePickerValue"
